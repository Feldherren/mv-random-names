 ...
 *
 * @param list1
 * @desc First list of names that can easily be used for multiple units; uses separator defined above.
 ...
 *
 * @param list2
 * @desc Second list of names that can easily be used for multiple units; uses separator defined above.
 ...
 * 
 * @help Random Names v1.0, by Feldherren (rpaliwoda@googlemail.com)
...
(function() {
 ...
 Game_Actor.prototype.setup = function(actorId) {
 	...
 	// read tags and create set of random names
 	// if no tags are picked up, name_set is populated with only the database name
 	var actor = $dataActors[actorId];
 	if (actor.meta.name_list)
 	{
 		name_set = parameters[actor.meta.name_list].split(parameters['Separator']);
 	}
 	else if (actor.meta.random_names)
 	{
 		name_set = actor.meta.random_names.split(parameters['Separator']);
 	}
 	else
 	{
 		name_set = [actor.name];
 	}
 	// Randomly pick name from name_set and change actor name accordingly
 ...
 Game_Enemy.prototype.setup = function(enemyId, x, y) {
 	...
 	// read tags and create set of random names
 	// if no tags are picked up, name_set is populated with only the database name
 	if (this.enemy().meta.name_list)
 	{
 		name_set = parameters[this.enemy().meta.name_list].split(parameters['Separator']);
 	}
 	else if (this.enemy().meta.random_names)
 	{
 		name_set = this.enemy().meta.random_names.split(parameters['Separator']);
 	}
 	else
 	{
 		name_set = [this.enemy().name];
 	}
 	// Randomly pick name from name_set and set random_name to it
 	var choice = Math.random();
 	choice *= name_set.length;
 	choice = Math.floor(choice);
 	random_name = name_set[choice];
 	this.random_name = random_name;
 };

// Replacing Game_Enemy.prototype.originalName to return random_name rather than just the enemy's name in the database.
